...
def linear(inp, output_dim, scope=None, stddev=1.0, reuse_scope=False):
    ...
    with tf.variable_scope(scope or "linear") as scope:
        ...
        w = tf.get_variable(
            "w", [inp.get_shape()[1], output_dim], initializer=norm, dtype=tf.float32
        )
...
class RNNGAN(object):
    ...
    def __init__(
        self,
        is_training,
        num_song_features=None,
        num_meta_features=None,
        conditioning="multi",
    ):
        ...
        self._input_songdata = tf.placeholder(
            shape=[None, songlength, num_song_features],
            dtype=tf.float32,
            name="input_data",
        )
        if conditioning is not "none":
            self._input_metadata = tf.placeholder(
                shape=[None, songlength, num_meta_features],
                dtype=tf.float32,
                name="input_metadata",
            )
            self._input_metadata_wrong = tf.placeholder(
                shape=[None, songlength, num_meta_features],
                dtype=tf.float32,
        ...
        if conditioning == "multi":
            metadata_inputs = [
                tf.squeeze(input_, [1])
                for input_ in tf.split(self._input_metadata, songlength, 1)
            ]
            wrong_metadata_inputs = [
                tf.squeeze(input_, [1])
        ...
        with tf.variable_scope("G") as scope:
            scope.set_regularizer(tf.contrib.layers.l2_regularizer(scale=REG_SCALE))
            g_cell_fw = make_rnn_cell(
                [HIDDEN_SIZE_G] * NUM_LAYERS_G,
                dropout_keep_prob=keep_prob,
                attn_length=ATTENTION_LENGTH,
            )
            g_cell_bw = make_rnn_cell(
                [HIDDEN_SIZE_G] * NUM_LAYERS_G,
                dropout_keep_prob=keep_prob,
                attn_length=ATTENTION_LENGTH,
            )
            ...
            random_rnninputs = tf.random_uniform(
                shape=[
                    batch_size,
                    songlength,
                    int(RANDOM_INPUT_SCALE * num_song_features),
                ],
                minval=0.0,
                maxval=1.0,
                dtype=tf.float32,
            ...
            if BIDIRECTIONAL_G:
                (
                    ...
                    g_cell_bw,
                    random_rnninputs,
                    initial_state_fw=self.g_initial_state_fw,
                    initial_state_bw=self.g_initial_state_bw,
                )
        ...
        self.g_params = [
            v for v in tf.trainable_variables() if v.name.startswith("model/G/")
        ]
        if ADAM:
        ...
        self.rnn_pretraining_loss = tf.reduce_mean(
            tf.squared_difference(
                x=tf.transpose(
                    tf.stack(self._generated_features_pretraining), perm=[1, 0, 2]
                ),
        ...
        pretraining_grads, _ = tf.clip_by_global_norm(
            tf.gradients(self.rnn_pretraining_loss, self.g_params), MAX_GRAD_NORM
        )
        ...
        with tf.variable_scope("D") as scope:
            ...
            if conditioning == "multi" and FEED_COND_D:
                conditioned_songdata_inputs = [
                    tf.concat([metadata_input, songdata_input], 1)
                    for metadata_input, songdata_input in zip(
                        metadata_inputs, songdata_inputs
                    )
                ]
            elif conditioning == "unique" and FEED_COND_D:
                conditioned_songdata_inputs = [
                    tf.concat([self._input_metadata, songdata_input], 1)
                    for songdata_input in songdata_inputs
                ]
            else:
                conditioned_songdata_inputs = [
                    tf.concat([songdata_input], 1) for songdata_input in songdata_inputs
                ]
            ...
            if conditioning == "multi" and FEED_COND_D:
                songdata_wrong_condition_inputs = [
                    tf.concat([wrong_metadata_input, songdata_input], 1)
                    for wrong_metadata_input, songdata_input in zip(
                        wrong_metadata_inputs, songdata_inputs
                    )
                ]
                self.wrong_d, self.wrong_d_features = self.discriminator(
                    songdata_wrong_condition_inputs,
                    cell_fw,
                    cell_bw,
                    is_training,
                ...
                songdata_wrong_condition_inputs = [
                    tf.concat([self._input_metadata_wrong, songdata_input], 1)
                    for songdata_input in songdata_inputs
                ]
                self.wrong_d, self.wrong_d_features = self.discriminator(
                    songdata_wrong_condition_inputs,
            ...
            if conditioning == "multi" and FEED_COND_D:
                generated_data = [
                    tf.concat([metadata_input, songdata_input], 1)
                    for metadata_input, songdata_input in zip(
                        metadata_inputs, self._generated_features
                    )
                ]
            elif conditioning == "unique" and FEED_COND_D:
                generated_data = [
                    tf.concat([self._input_metadata, songdata_input], 1)
                    for songdata_input in self._generated_features
                ]
            else:
                generated_data = [
                    tf.concat(songdata_input, 1)
                    for songdata_input in self._generated_features
                ]
        ...
        if conditioning == "none" or LOSS_WRONG_D is False:
            self.d_loss = tf.reduce_mean(
                -tf.log(tf.clip_by_value(self.real_d, 1e-1000000, 1.0))
                - tf.log(1 - tf.clip_by_value(self.generated_d, 0.0, 1.0 - 1e-1000000))
            )
        else:
            self.d_loss = tf.reduce_mean(
                -2 * tf.log(tf.clip_by_value(self.real_d, 1e-1000000, 1.0))
                - tf.log(1 - tf.clip_by_value(self.generated_d, 0.0, 1.0 - 1e-1000000))
        ...
        self.g_loss = tf.reduce_mean(
            tf.log(tf.clip_by_value(1 - self.generated_d, 1e-1000000, 1.0))
        )
        ...
        d_grads, _ = tf.clip_by_global_norm(
            tf.gradients(self.d_loss, self.d_params), MAX_GRAD_NORM
        )
        self.opt_d = d_optimizer.apply_gradients(zip(d_grads, self.d_params))
        if FEATURE_MATCHING:
            g_grads, _ = tf.clip_by_global_norm(
                tf.gradients(self.g_loss_feature_matching, self.g_params), MAX_GRAD_NORM
            )
        else:
            g_grads, _ = tf.clip_by_global_norm(
                tf.gradients(self.g_loss, self.g_params), MAX_GRAD_NORM
            )
        self.opt_g = g_optimizer.apply_gradients(zip(g_grads, self.g_params))

        self._new_lr = tf.placeholder(
            shape=[], name="new_learning_rate", dtype=tf.float32
        )
        self._lr_update = tf.assign(self._lr, self._new_lr)

    def discriminator(
        self, inputs, cell_fw, cell_bw, is_training, msg="", keep_prob=1.0
    ):
        """
        RNN discriminator
        """
        ...
        decision = tf.Print(
            decision, [decision], "{} decision = ".format(msg), first_n=20
        )
...
def run_epoch(
    ...
    if MULTI and CONDITION:
        [batch_song, batch_meta, batch_meta_wrong, pointer] = utils.get_batch_multi(
            BATCH_SIZE,
            pointer,
            train,
            validate,
        ...
        [batch_song, batch_meta, batch_meta_wrong, pointer] = utils.get_batch(
            BATCH_SIZE,
            pointer,
            train,
            validate,
        ...
        [batch_song, pointer] = utils.get_batch_no_cond(
            BATCH_SIZE,
            pointer,
            train,
            validate,
...
def main():
    ...
    with tf.Graph().as_default(), tf.Session(config=config) as session:
        ...
        for i in range(global_step, MAX_EPOCH):
            ...
            print(
                "Epoch: {} Learning rate: {:.3f}, pretraining: {}".format(
                    i, session.run(m.lr), i < PRETRAINING_EPOCHS
                )
            )
            ...
            if i < PRETRAINING_EPOCHS:
                ...
                if PRETRAINING_D:
                    try:
                        print(
                            "Epoch: {} Pretraining loss: G: {:.3f}, D: {:.3f}".format(
                ...
                try:
                    print(
                        "Epoch: {} Train loss: G: {:.3f}, D: {:.3f}".format(
                            i, train_g_loss, train_d_loss
                        )
                    )
                except:
                    print(
                        "Epoch: {} Train loss: G: {}, D: {}".format(
                            i, train_g_loss, train_d_loss
                        )
                    )
            ...
            valid_g_loss, valid_d_loss = run_epoch(
                session, m, train, validate, test, "validation", tf.no_op(), tf.no_op()
            )
            try:
                print(
                    "Epoch: {} Valid loss: G: {:.3f}, D: {:.3f}".format(
                        i, valid_g_loss, valid_d_loss
                    )
                )
            except:
                print(
                    "Epoch: {} Valid loss: G: {}, D: {}".format(
                        i, valid_g_loss, valid_d_loss
                    )
                )
            ...
            if i % 5 == 0:
                ...
                try:
                    shutil.rmtree(
                        "./saved_gan_models/epoch_models/model_epoch" + str(i)
                    )
                except:
                    a = 0
                output = tf.convert_to_tensor(
                    m.generated_features, name="output_midi", dtype=tf.float32
                )
                    ...
                    builder.add_meta_graph_and_variables(
                        ...
                        signature_def_map={
                            "output": tf.saved_model.signature_def_utils.predict_signature_def(
                                inputs={
                                    m.keep_prob.name: m.keep_prob,
                                    m.input_songdata.name: m.input_songdata,
            ...
            feed_dict = {}
            feed_dict[m.input_songdata] = np.random.uniform(
                size=(1, SONGLENGTH, NUM_MIDI_FEATURES)
            )
            ...
            if CONDITION:
                ...
                if not MULTI:
                    ...
                    condition.append(
                        np.split(test[0][NUM_MIDI_FEATURES * SONGLENGTH :], SONGLENGTH)
                    )
                feed_dict[m.input_metadata.name] = condition

            print(
                "EXAMPLE DATASET SONG MIDI PARAMETERS=============================================================\n"
            )
            example_song = []
            ...
            for iters in range(SONGLENGTH):
                example_song.append(
                    test[0][
                        NUM_MIDI_FEATURES * iters : NUM_MIDI_FEATURES * iters
                        + NUM_MIDI_FEATURES
                    ]
                )
            print(example_song)
            ...
            print(
                "AVERAGE STATS OVER {} GENERATED SONGS==========================================================\n".format(
                    np.shape(test)[0]
                )
            )
            ...
            for j in range(0, np.shape(validate)[0]):
                ...
                if CONDITION:
                    ...
                    if MULTI:
                        condition.append(
                            np.split(
                                validate[j][NUM_MIDI_FEATURES * SONGLENGTH :],
                                SONGLENGTH,
                            )
                        )
                    else:
                ...
                if NUM_MIDI_FEATURES == 3:
                    ...
                    model_stats["stats_repetitions_2_tot"] += float(
                        stats["repetitions_2"]
                    )
                    model_stats["stats_repetitions_3_tot"] += float(
                        stats["repetitions_3"]
                    )
                    model_stats["stats_unique_tones_tot"] += float(
                        stats["tones_unique"]
                    )
                    ...
                    model_stats["best_scale_score"] = max(
                        stats["scale_score"], model_stats["best_scale_score"]
                    )
                    model_stats["best_repetitions_2"] = max(
                        stats["repetitions_2"], model_stats["best_repetitions_2"]
                    )
                    model_stats["best_repetitions_3"] = max(
                        stats["repetitions_3"], model_stats["best_repetitions_3"]
                    )
                    ...
                    if (
                        stats["scale_score"] == 1.0
                        and stats["tones_unique"] > 3
                        and stats["tone_span"] > 4
                        and stats["num_null_rest"] > 8
                        and stats["tone_span"] < 13
            ...
            if NUM_MIDI_FEATURES == 3:
                ...
                for i in range(SONGLENGTH):
                    ...
                    val_dat_duration[:, i] = np.array(validate)[
                        :, NUM_MIDI_FEATURES * i + 1
                    ]
                    val_dat_rests[:, i] = np.array(validate)[
                        :, NUM_MIDI_FEATURES * i + 2
                    ]
                ...
                if MMD_pitch < MMD_pitch_old:
                    ...
                    builder = tf.saved_model.builder.SavedModelBuilder(
                        "./saved_gan_models/saved_model_best_pitch_mmd"
                    )
                    builder.add_meta_graph_and_variables(
                        session,
                        [],
                ...
                if MMD_duration < MMD_duration_old:
                    ...
                    try:
                        shutil.rmtree(
                            "./saved_gan_models/saved_model_best_duration_mmd"
                        )
                    except:
                        a = 0
                    output = tf.convert_to_tensor(
                ...
                if MMD_rest < MMD_rest_old:
                    ...
                    builder = tf.saved_model.builder.SavedModelBuilder(
                        "./saved_gan_models/saved_model_best_rest_mmd"
                    )
                    builder.add_meta_graph_and_variables(
                        session,
                        [],
                ...
                if ATTRIBUTE == "length":
                    val_gen_duration = np.zeros(
                        (np.shape(validation_songs)[0], SONGLENGTH)
                    )
                    val_dat_duration = np.zeros((np.shape(validate)[0], SONGLENGTH))
                    for i in range(SONGLENGTH):
                ...
                if ATTRIBUTE == "rest":
                    ...
                    mmd_rest_list.append(MMD_rest)
                if ATTRIBUTE == "pitch":
                    val_gen_pitches = np.zeros(
                        (np.shape(validation_songs)[0], SONGLENGTH)
                    )
                    val_dat_pitches = np.zeros((np.shape(validate)[0], SONGLENGTH))
                    for i in range(SONGLENGTH):
                        val_gen_pitches[:, i] = np.array(validation_songs)[:, i, 0]
                        val_dat_pitches[:, i] = np.array(validate)[
                            :, NUM_MIDI_FEATURES * i
                        ]
        ...
        builder = tf.saved_model.builder.SavedModelBuilder(
            "./saved_gan_models/saved_model_end_of_training"
        )
        if CONDITION:
            builder.add_meta_graph_and_variables(
                session,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Train a GAN to generate sequential, real-valued data."
    )
    parser.add_argument(
        "--settings_file",
        help="json file of settings, overrides everything else",
        type=str,
        default="settings",
    )
